...
namespace StreamInsights.WebSocketUtility
{  
    public class CustomClientWebSocketOption
    {
        ...
        public CustomClientWebSocketOption()
        {
    ...
    public sealed class CustomClientWebSocket : WebSocket
    {
        private readonly CustomClientWebSocketOption options;
        ...
        public CustomClientWebSocketOption Options
        {
            get
            {
                return this.options;
        ...
        {
            this.state = 0;
            this.options = new CustomClientWebSocketOption();
        ...
        private HttpWebRequest CreateAndConfigureRequest(Uri uri)
        {
            ...
            foreach (string name in this.options.RequestHeaders.Keys)
            {
                ...
                {
                    httpWebRequest.Connection = this.options.RequestHeaders[name];
                }
                else if (name.Equals("Timeout", StringComparison.OrdinalIgnoreCase))
                {
                    httpWebRequest.Timeout = Convert.ToInt32(options.RequestHeaders[name]);
                }
                else
        ...
        private string ValidateResponse(HttpWebRequest request, HttpWebResponse response)
        {
            ...
            if (!string.Equals(text2, "Upgrade,Keep-Alive", StringComparison.OrdinalIgnoreCase))
            {
                throw new WebSocketException("net_WebSockets_InvalidResponseHeader");
            }
            string text3 = response.Headers["Sec-WebSocket-Accept"];
            string secWebSocketAcceptString = CustomClientWebSocketOption.GetSecWebSocketAcceptString(request.Headers["Sec-WebSocket-Key"]);
